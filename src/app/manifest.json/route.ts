...
export async function GET() {
  try {
    ...
    // Get icon URLs from media library
    let icon512Url = `${siteUrl}/icon-512.png`;
    let icon192Url = `${siteUrl}/icon-192.png`;
    ...
    if (settings.iconId) {
      ...
      if (icon512) icon512Url = `${siteUrl}${icon512.path}`;
    }
    ...
    if (settings.icon192Id) {
      ...
      if (icon192) icon192Url = `${siteUrl}${icon192.path}`;
    }
    ...
    const fallback = {
      ...
      icons: [
        {
          src: `${fallbackUrl}/icon-192.png`,
          sizes: '192x192',
          type: 'image/png',
        },
        {
          src: `${fallbackUrl}/icon-512.png`,
          sizes: '512x512',
          type: 'image/png',
        },
      ],
